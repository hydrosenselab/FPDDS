...
def mape(evaluation, simulation):
    """
    ...
    if len(evaluation) == len(simulation) > 0:
        obs, sim = np.array(evaluation), np.array(simulation)
        mape = np.mean((np.abs(sim-obs))/obs)*100
        return mape
    else:
        logging.warning("evaluation and simulation lists does not have the same length.")
        return np.nan

def nmae(evaluation, simulation):
    """
    ...
    if len(evaluation) == len(simulation) > 0:
        obs, sim = np.array(evaluation), np.array(simulation)
        nmae = ((np.mean(np.abs(sim - obs)))/np.mean(obs))
        return nmae
    else:
        logging.warning("evaluation and simulation lists does not have the same length.")
        return np.nan
...
def rrmse(evaluation, simulation):
    ...
    if len(evaluation) == len(simulation):
        obs, sim = np.array(evaluation), np.array(simulation)
        rrmse = (rmse(evaluation, simulation) / np.mean(obs))
        return rrmse
    else:
        logging.warning("evaluation and simulation lists does not have the same length.")
        return np.nan
...
def kge(evaluation, simulation, return_all=True):
    """
    ...
    if len(evaluation) == len(simulation):
        ...
        kge_wr = 1 - np.sqrt((cc - 1)**2 + 5*(alpha - 1)**2 + (beta - 1)**2)
        kge = 1 - np.sqrt((cc - 1)**2 + (alpha - 1)**2 + (beta - 1)**2)
        if return_all:
            return kge_wr, kge, cc, alpha, beta
        else:
            return kge
    else:
        logging.warning("evaluation and simulation lists does not have the same length.")
...
def R2(evaluation, simulation):
    """
    Computes the coefficient of determination (R²) between evaluation and simulation data.

    :param evaluation: Array-like, observed values
    :param simulation: Array-like, simulated values
    :return: R² value
    """
    evaluation = np.asarray(evaluation)
    simulation = np.asarray(simulation)
    
    covariance = np.dot(evaluation - np.mean(evaluation), simulation - np.mean(simulation)) / len(evaluation)
    correlation = covariance / (np.std(evaluation) * np.std(simulation))
